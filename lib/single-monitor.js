...
const checkMiniRangesDate = (since, until, days, queue) => { // LA FECHA SE DEBERIA AVANZAR CUANDO YA SE HIZO UNA EJECUCION DE TODAS LAS CUENTAS.
    ...
    if (queueLastRangeExecute.length === 0) {
        ...
        if (until.getTime() >= prox_until.getTime()) {
            newUntil = new Date(queue_until_cp.setDate(queue_until_cp.getDate() + days));
        } else {
            newUntil = until;
        }
        ...
        async (queue) => {
            promises.push(
                new Promise(async (resolve,_) => {
                    ...
                    logger(`   * Queue ${queue.name} eliminada.`);
                    resolve();
                })
            )
        }
...
const nextRangeDate = (sinceDate, untilDate, queue) => {
    let rangeSince = sinceDate;
    let rangeUntil = untilDate;
    if (queue.lastRangeExecute.length === 0) {
        if (COUNT_OF_DAYS > 0) {
            let [newSince, newUntil] = checkMiniRangesDate(sinceDate, untilDate, COUNT_OF_DAYS, queue);
...
const generateFirstRange = (since, until, queue, days) => {
    let rangeSince = since;
    let rangeUntil = until;
    if (queue.lastRangeExecute.length === 0) {
        let since_cp = new Date(since.getTime());
...
const checkPosts = async (facebookAPI, _accountBuffer) => {
    ...
    if (await canResetQueues(queues)) {
        ...
        queues = await resetQueues(queues, campaigns);
        let [rangeSince, rangeUntil] = nextRangeDate(currentCampaign.startDate, currentCampaign.endDate, queue);
        miniRangeSince = rangeSince;
        miniRangeUntil = rangeUntil;
        process.exit();
    }
