...
const markAccountAsProcessed = (account, accountCampaign, campaignQueue, quarentine=false) => {
...
const getPostFromFacebook = async (facebookAPI, account, accountCampaign, lastPostDate, campaignQueue) => {
    try {
        ...
        if (posts) {
            ...
            markAccountAsProcessed(account, accountCampaign, campaignQueue);
        }        
    } catch (err) {
...
const getCampaignQueue = async (campaign) => {
    ...
    return queue !== null ? await queue.populate('nonProcessed', { 'sort': [['lastDateProcessed', 'asc']] }).execPopulate() : queue;
};
                                ...
                                async(account, idx) => {
                                    processedPromises.push(
                                        new Promise(
                                            ...
                                            logger(`   * Procesando cuenta: ${account} en ${accountCampaign} `);    
