...
const analyzeError = async (err, account, campaign, campaignQueue) => {
    ...
    if (isApiLimitError(error)) {
        ...
        markAccountAsProcessed(account, campaign, campaignQueue);
    }
...
const markAccountAsProcessed = (account, accountCampaign, campaignQueue) => {
    ...
    account.save();
    // sacar del array de nonProcessed de la campaignQueue.
...
const createCampaignQueue = async (campaign) => {
    ...
    logger(`   * Queue ${newQueue.name} craeda con éxito.`);
    return queue;
}
