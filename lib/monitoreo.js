...
const getPostFromFacebook = async (facebookAPI, account, accountCampaign, lastPostDate, campaignQueue) => {
    try {
        ...
        if (posts) {
                ...
                async (p) => {
                    ...
                    if (post.full_picture) {
                        try {
                            ...
                            logger(`    * Falló descarga de imagen del post: ${post.id} cuenta: ${account.name} imagen: ${post.full_picture}\n
                                  Error: ${error}\n`)
                        }
                    }
            ...
            markAccountAsProcessed(account, accountCampaign, campaignQueue, false, postsSaved);
        }        
    } catch (err) {
...
const resetQueues = async (queues, campaigns) => {
    const promises = [];
    queues.forEach(
        async (queue) => {
            promises.push(
    ...
    return await verifyQueues(campaigns)
};
...
const checkPosts = async (facebookAPI, _accountBuffer) => {
    ...
    if (campaigns.length >= 1) {
        let queues = await verifyQueues(campaigns);
        if (await canResetQueues(queues)) {
            logger('    * Reiniciando Queues...')
